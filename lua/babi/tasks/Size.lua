...
function Size:generate_story(world, knowledge, story, config)
    ...
    for i = 1, config.steps do
        local x, y = objects[initial_objects[i]], objects[initial_objects[i + 1]]
        local clause = Clause(world, true, world:god(), actions.set,
                              x, '>', y)
        story:append(clause)
        ...
        while true do
            ...
            if (not ordering.edges[x] or not ordering.edges[x][y]) then
                ...
                if #tablex.keys(options) == 0 then
                    ...
                    local clause = Clause(world, true, world:god(), actions.set, x, '>', y)
                    story:append(clause)
    ...
    for pair, support in pairs(options) do
        local clause = Clause(world, true, world:god(), actions.set,
                              pair[1], '>', pair[2])
        story:append(Question('eval', clause, Set(support)))
    end
