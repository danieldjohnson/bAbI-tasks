...
function Time:generate_story(world, knowledge, story)
    ...
    local times = {'yesterday', 'this morning', 'this afternoon', 'this evening'}
    ...
    story[i] = Clause(world, true, actor, actions.teleport,
                      first_locations[1], 'when', times[first_times[1]])
    movements[actor][first_times[1]] = story[i]
    story[j] = Clause(world, true, actor, actions.teleport,
    ...
    for i = 1, num_questions * 3 + 2 do
        if i > 2 and (i - 2) % 3 == 0 then
            ...
            repeat
                ...
                actor_movements = movements[actor]:filter(function(clause) return not not clause end)
            ...
            story[i] = Question(
                'eval',
                Clause(world, true, world:god(), actions.set, actor, 'before',
                       actor_movements[t].args[1],
                       actor_movements[t + 1].args[1]),
        ...
        elseif not story[i] then
            ...
            story[i] = Clause(world, true, actor, actions.teleport, location,
                              'when', times[time])
